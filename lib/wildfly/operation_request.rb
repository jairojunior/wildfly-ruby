require 'base64'
require 'wildfly/client'
require 'wildfly/operation_builder'
...
module Wildfly
  class OperationRequest
    ...
    def exec(command, ignore_failed_outcome = false)
      ...
      @client.submit(detyped_request, ignore_failed_outcome)
    end
    ...
    def exists?(resource)
      response = @client.submit(operation.read(resource).build, :ignore_failed_outcome => true)
      response['outcome'] == 'success'
    end

    def read(resource, recursive = false)
      response = @client.submit(operation.read(resource).with(:recursive => recursive).build)
      response['result']
    end
    ...
    def add(resource, state, recursive, headers)
      if recursive
        ...
        @client.submit(operation.composite(*operations).headers(headers).build)
      else
        @client.submit(operation.add(resource).with(state).headers(headers).build)
      end
    end
    ...
    def update(resource, state, recursive, headers)
      if recursive
        ...
        @client.submit(operation.composite(*diff(state, resource)).headers(headers).build)
      end
    end
    ...
    def remove(resource, headers)
      @client.submit(operation.remove(resource).headers(headers).build)
    end
    ...
    def deploy(name, source, server_group, headers)
      @client.submit(operation.composite(*deploy_operations(name, source, server_group)).headers(headers).build)
    end
    ...
    def undeploy(name, server_group, headers)
      @client.submit(operation.composite(*undeploy_operations(name, server_group)).headers(headers).build)
    end
    ...
    def update_deploy(name, source, server_group, headers)
      @client.submit(operation.composite(*update_deploy_operations(name, source, server_group)).headers(headers).build)
    end
