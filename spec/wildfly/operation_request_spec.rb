require 'spec_helper'
require 'wildfly/operation_request'

describe Wildfly::OperationRequest do
  context 'when performing requests' do
    ...
    it 'executes an arbitrary command' do
      ...
      allow(api_client).to receive(:submit).with(Wildfly::CLICommand.new(command).to_detyped_request, false)
      request.exec(command)
    end

    it 'reads a resource' do
      resource = '/subsystem=web'
      allow(api_client).to receive(:submit).with(Wildfly::OperationBuilder.new.read(resource).with(:recursive => false).build).and_return('result' => {})
      request.read(resource)
    end

    it 'removes a resource' do
      resource = '/subsystem=web'
      allow(api_client).to receive(:submit).with(Wildfly::OperationBuilder.new.remove(resource).headers({}).build)
